...
# Stats storage - DynamoDB with in-memory fallback
class StatsStore:
    def __init__(self):
        self.dynamodb = None
        self.table = None
        ...
        self._dynamodb_initialized = False
    ...
    def _ensure_dynamodb_connection(self):
        ...
        table_name = os.getenv('DYNAMODB_TABLE', 'qr-stats')
        try:
            ...
            logging.warning(f"DynamoDB not available, using in-memory storage: {e}")
            self.dynamodb = None
            self.table = None
    ...
    def increment_qr_count(self):
        ...
        if self.table:
            try:
                ...
                hour_expires = int((now + timedelta(hours=1)).timestamp())
                day_expires = int((now + timedelta(days=1)).timestamp())
                week_expires = int((now + timedelta(weeks=1)).timestamp())
                
                # Increment counters in DynamoDB
                self.table.update_item(
                    Key={'stat_key': hour_key},
                    UpdateExpression='ADD #count :inc SET expires_at = :expires',
                    ExpressionAttributeNames={'#count': 'count'},
                    ExpressionAttributeValues={':inc': 1, ':expires': hour_expires}
                )
                self.table.update_item(
                    Key={'stat_key': day_key},
                    UpdateExpression='ADD #count :inc SET expires_at = :expires',
                    ExpressionAttributeNames={'#count': 'count'},
                    ExpressionAttributeValues={':inc': 1, ':expires': day_expires}
                )
                self.table.update_item(
                    Key={'stat_key': week_key},
                    UpdateExpression='ADD #count :inc SET expires_at = :expires',
                    ExpressionAttributeNames={'#count': 'count'},
                    ExpressionAttributeValues={':inc': 1, ':expires': week_expires}
                )
            except Exception:
                # DynamoDB failed, fall back to memory
    ...
    def get_stats(self):
        ...
        if self.table:
            try:
                ...
                hour_item = self.table.get_item(Key={'stat_key': hour_key}).get('Item', {})
                day_item = self.table.get_item(Key={'stat_key': day_key}).get('Item', {})
                week_item = self.table.get_item(Key={'stat_key': week_key}).get('Item', {})
                
                stats["last_hour"] = int(hour_item.get('count', 0))
                stats["last_day"] = int(day_item.get('count', 0))
                stats["last_week"] = int(week_item.get('count', 0))
            except Exception:
                # DynamoDB failed, use memory
                ...
                stats["storage_type"] = "memory (dynamodb failed)"
        else:
            stats["last_hour"] = self.memory_store.get(hour_key, 0)
            stats["last_day"] = self.memory_store.get(day_key, 0)
            stats["last_week"] = self.memory_store.get(week_key, 0)
...
@app.route("/api/test")
def test_dynamodb():
    """Test endpoint to verify DynamoDB connectivity and basic functionality"""
    stats_store._ensure_dynamodb_connection()
    
    test_result = {
        "dynamodb_connected": stats_store.storage_type == "dynamodb",
        "storage_type": stats_store.storage_type,
        "timestamp": datetime.now().isoformat()
    }
    
    # Test DynamoDB operations if connected
    if stats_store.table:
        try:
            ...
            # Put test item
            stats_store.table.put_item(Item={
                'stat_key': test_key,
            ...
            # Get test item
            response = stats_store.table.get_item(Key={'stat_key': test_key})
            retrieved_item = response.get('Item', {})
            
            # Delete test item
            stats_store.table.delete_item(Key={'stat_key': test_key})
            
            test_result["dynamodb_test"] = {
                "put_get_test": "passed" if retrieved_item.get('test_value') == 'test_data' else "failed",
